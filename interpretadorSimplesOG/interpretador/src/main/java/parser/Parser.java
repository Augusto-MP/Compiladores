...
public class Parser {
    ...
    Access offset(Id a) throws IOException { // I -> [E] | [E] I
        ...
        Type type = a.type;
        match('['); i = bool(); match(']'); // primeiro índice, I -> [ E ]
        type = ((Array)type).of;
        w = new Constant(type.width);
        t1 = new Arith(new Token('*'), i, w);
        loc = t1;
        while( look.tag == '[' ) { // I multidimensional -> [ E ] I
            match('['); i = bool(); match(']');
            type = ((Array)type).of;
            w = new Constant(type.width);
            t1 = new Arith(new Token('*'), i, w);
    ...
    public int getUsed() { return used; } // retorna memória usada
